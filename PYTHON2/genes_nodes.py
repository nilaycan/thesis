...
#outfile=open("sonucabak.txt","w")
#for k in sorted(mp2id.values()):
#        outfile.write(str(k)+"\n")
#outfile.write("\n")
#for k in sorted(gene2geneId.values()):
#        outfile.write(str(k)+"\n")
...
for humandisease in humandis2gene.keys():
    ...
    #current_att2="# of Mouse Genes: "+str(len(humandis2gene[humandisease]))+" AND "+str(len(missing)-prev_missing_length)+" of them do/does not have any MP Id on Jax Database"+". "+" Genes are " + ', '.join(current_gene_list)
    current_att2="This network aims to reveal the OMIM disorders and mouse affected system connections by using ortholog mouse knock out diseasome genes as edges. The size of disease and affected system nodes are respectively proportional to the number of genes in it and number of disease it connects. Black nodes represent affected systems. Red nodes represents mouse genes and other colours show distinct 22 disorder classes."   
    ...
    for gene_tuple in humandis2gene[humandisease]:
        ...
        try:
            ...
            for affected in gene2affected[gene]:
                current_label, current_node_id,mp1,affected1,mp2,affected2=affected
                current_size=str(50.0)
                #current_label, current_node_id=str(current_label), str(current_node_id)
                current_att1="Child affected system"
                current_att2=current_label
                #angle=random.uniform(-3.14*2,3.14*2)
                #radius=random.uniform(0,r)
                #current_pos=str(radius*math.cos(angle)),str(radius*math.sin(angle))
                #current_col=str(68),str(68),str(238)
...
for humandisease in humandis2gene.keys():
    ...
    first_gene, current_att2, current_dis_source_node_id=str(current_gene),str(current_disclass),str(current_row)
    for gene_tuple in humandis2gene[humandisease]:
        gene,current_disclass,current_row=gene_tuple
        try:
            dummy=len(gene2affected[gene])
            ...
            for affected in gene2affected[gene]:
                ...
                current_target_node_id=str(mp2id[current_mp])
                current_edge_string='      <edge id="'+str(edge_counter)+'" source="'+current_source_node_id+'" target="'+current_target_node_id+'" label="'+gene+'">\n'
                current_edge_string=current_edge_string+'        <attvalues></attvalues>\n      </edge>\n'
                edge_counter=edge_counter+1
                edgedic[current_edge_string]=1.0
            '''
            for affected in gene2affected[gene]:
                ...
                try:
                    ...
                    current_edge_string='      <edge id="'+str(edge_counter)+'" source="'+current_source_node_id+'" target="'+current_parent1_target_node_id+'">\n'
                    current_edge_string=current_edge_string+'        <attvalues></attvalues>\n      </edge>\n'
                    edge_counter=edge_counter+1
                    edgedic[current_edge_string]=1.0
                if mp2!="noparent":
                    try:
                        ...
                        current_edge_string='      <edge id="'+str(edge_counter)+'" source="'+current_source_node_id+'" target="'+current_parent2_target_node_id+'">\n'
                        current_edge_string=current_edge_string+'        <attvalues></attvalues>\n      </edge>\n'
                        edge_counter=edge_counter+1
                        edgedic[current_edge_string]=1.0
            '''
